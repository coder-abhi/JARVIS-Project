...
export default function GoalsPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f4f6f3] pb-36 text-stone-950">
      <section className="border-b border-stone-200 bg-[linear-gradient(135deg,#fff8ed_0%,#eefaf7_58%,#f7edf6_100%)]">
        <div className="mx-auto max-w-7xl px-5 py-8 md:px-8 md:py-10">
          <p className="text-sm font-semibold uppercase tracking-wide text-teal-700">Goals command center</p>
          <div className="mt-3 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h1 className="text-4xl font-semibold leading-tight text-stone-950 md:text-5xl">Choose the next right move.</h1>
              <p className="mt-4 max-w-2xl text-sm leading-6 text-stone-700">
                Keep tasks tied to the horizon that actually matters: monthly execution, quarterly milestones, yearly direction, and five-year identity.
              </p>
            </div>
            <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-3 md:min-w-[420px]">
              <Metric label="Open tasks" value={overview?.active_tasks.length ?? 0} />
              <Metric label="Goals" value={overview?.goals.length ?? 4} />
              <Metric label="Done recently" value={overview?.recent_completed_tasks.length ?? 0} />
            </div>
          </div>
        </div>
      </section>

      <div className="mx-auto grid max-w-7xl gap-4 px-5 py-6 md:px-8">
        {error ? <p className="rounded-lg bg-red-50 p-3 text-xs font-semibold text-red-700">{error}</p> : null}
        {message ? <p className="rounded-lg bg-emerald-50 p-3 text-xs font-semibold text-emerald-800">{message}</p> : null}

        <section className="rounded-lg border border-stone-200 bg-white/80 p-4 shadow-sm">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-teal-700">Need to complete</p>
              <h2 className="mt-1 text-lg font-semibold">Task list</h2>
            </div>
            <div className="flex flex-wrap gap-1 rounded-full bg-stone-100 p-1 text-xs font-semibold text-stone-600">
              ...
              <SortButton active={sortMode === "goal"} onClick={() => setSortMode("goal")} label="Goal" />
            </div>
          </div>
          ...
          <div className="mt-4 overflow-hidden rounded-md border border-stone-200 bg-white">
            {isLoading ? (
              ...
              <>
                ...
                {sortedTasks.map((task) => (
                  <div key={task.id} className="grid gap-1 border-b border-stone-100 px-3 py-2 text-xs transition last:border-b-0 hover:bg-teal-50/35 md:grid-cols-[32px_minmax(0,1fr)_72px_92px_minmax(120px,0.75fr)_minmax(100px,0.7fr)] md:items-center md:gap-3">
                    ...
                    <span className="min-w-0 truncate text-amber-800">{task.goal_title ?? "General"}</span>
              ...
              <div className="bg-stone-50 p-5 text-center">
                <h3 className="text-sm font-semibold text-stone-950">No open tasks yet</h3>
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_0.9fr]">
          <div className="rounded-lg border border-stone-200 bg-white/80 p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-teal-700">AI Next Goals</p>
                <h2 className="mt-1 text-lg font-semibold">Suggested next actions</h2>
              </div>
              <button
                type="button"
        ...
        <section className="rounded-lg border border-stone-200 bg-white/80 p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-teal-700">Recently Completed Tasks</p>
          <h2 className="mt-1 text-lg font-semibold">Completion log</h2>
          <div className="mt-4 overflow-hidden rounded-md border border-stone-200 bg-white">
      ...
      <form onSubmit={handleLogSubmit} className="fixed inset-x-0 bottom-0 z-40 border-t border-stone-200 bg-[#f4f6f3]/95 px-4 py-2.5 shadow-2xl shadow-stone-950/20 backdrop-blur">
        <div className="mx-auto flex max-w-4xl items-center gap-2 rounded-full border border-stone-200 bg-white px-3 py-1.5 shadow-lg shadow-stone-900/10">
          <input
            ...
            placeholder="+ Add task or - log completed work"
  ...
  return (
    <div className="rounded-lg border border-stone-200 bg-white/80 p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-teal-700">{categoryLabels[category]}</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="grid h-6 w-6 place-items-center rounded-full bg-teal-50 text-xs font-semibold text-teal-800">
            {goals.length}
          </span>
          <button
            type="button"
      ...
      <div className="mt-3 grid gap-2">
        {goals.length ? (
          ...
          >
            Add your first {categoryLabels[category].toLowerCase()}.
          </button>
        )}
      </div>
    </div>
  ...
  return (
    <button type="button" onClick={onBeginEdit} className="rounded-lg border border-stone-200 bg-white p-3 text-left transition hover:border-teal-200 hover:bg-teal-50/30">
      ...
      {goal.measurable && goal.progress_percentage != null ? (
        ...
        <p className="mt-2 text-xs font-medium text-stone-500">Click to edit or add a measurable target.</p>
      )}
    </button>
  );
}
...
function PersonalityPanel({
  ...
  return (
    <div className="rounded-lg border border-stone-200 bg-white/80 p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-teal-700">Personality Insight</p>
          <h2 className="mt-1 text-lg font-semibold">Goal pattern analysis</h2>
        </div>
      ...
      <p className="mt-4 text-xs leading-5 text-stone-700">
        {insight?.text ?? "No personality analysis yet. Refresh when you want the LLM to analyze your goals and recent completions."}
      </p>
...
function SortButton({ active, onClick, label }: { active: boolean; onClick: () => void; label: string }) {
  return (
    <button type="button" onClick={onClick} className={`rounded-full px-3 py-1.5 transition ${active ? "bg-white text-stone-950 shadow-sm" : "hover:bg-white/70 hover:text-stone-950"}`}>
      {label}
    </button>
  );
}
